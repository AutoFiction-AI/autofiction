...
4e. Shared pattern IDs that are especially relevant to this prompt include `READER_MODEL_MISCAL` for book-wide over- or under-exposition relative to genre and `VOICE_CONVERGENCE` for cast-wide tonal flattening where multiple named characters drift into the same register without style-bible support.
...
6g. Flag reader-model miscalibration as MEDIUM or HIGH per affected chapter when the book either over-explains commonsense material the genre treats as baseline noise or under-explains novel-specific concepts the cold reader cannot infer. Use `outline/style_bible.json`, especially `prose_style_profile.exposition_density_policy`, as the calibration target, but judge the manuscript on what the reader experiences on the page. When this is a recurring book-wide tendency, prefer a `pattern_findings` entry with `pattern_id: READER_MODEL_MISCAL`.
